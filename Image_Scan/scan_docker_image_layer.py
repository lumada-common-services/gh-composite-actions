...
class ContainerImageScanner():
    ...
    def generate_project_structures(self, base_layers=None):
        ...
        for layer in self.layers:
            parameters = {}
            if 'created_by' in layer['command']:
    ...
    def submit_layer_scans(self):
        for layer in self.layers:
            ...
            ProjectName,SnippetMatching=get_values()
            print("ProjectName")
            print(ProjectName)
            print("SnippetMatching")
            print(SnippetMatching)

            if "layer_1_" in layer['name']:
                print("found layer_1: " + layer['name'])
                option="--detect.project.name=" + ProjectName + "-OS"
                options.append(option)
                
            elif layer['name'].endswith("layer_1"):
                print("found layer_1: " + layer['name'])
                option="--detect.project.name=" + ProjectName + "-OS"                
                options.append(option)

            else:
                option="--detect.project.name=" + ProjectName + "-APP"
                options.append(option)
                if SnippetMatching == "False":
                    option="--detect.blackduck.signature.scanner.snippet.matching=NONE"
                else:
                    option="--detect.blackduck.signature.scanner.snippet.matching=SNIPPET_MATCHING"
                options.append(option)                    

            options.append('--detect.project.version.name=layer-1')
...
def main(argv=None):
    ...
    parser.add_argument('projectname', type=str, help="Project Name should be defined. e.g. dataflow-manager")        
    parser.add_argument('snippet.matching', default="False", type=str, help="Snippet Matching should be set to True if Snippet Matching has to be enabled")
